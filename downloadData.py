...
for year in yearData:
	for month in monthData:
		print(f'{year}-{month}')
		url = 'https://climate.weather.gc.ca/climate_data/bulk_data_e.html?format=csv&stationID='+str(stationID)+'&Year='+str(year)+'&Month='+str(month)+'&Day='+str(dayData)+'&timeframe='+str(timeframe)+'&submit=Download+Data'
		# Add option to do UTC or LST
		source = urllib.request.urlopen(url).read()
		# print(url)
		# sys.exit()

		filename = 'data/'+stationName+'/stationID_'+str(stationID)+'year_'+str(year)+'_month_'+str(month)+'.csv'
